...
class HumanOverlay:
    def __init__(self, motion):
        ...
        self.mesh_renderer = SkeletalMeshRenderer(
            pipeline_utils.get_source_model_mesh(pipeline_utils.SourceType.SOMA, skeleton)
        )
    ...
    def draw(
        ...
        *,
        ...
        self.mesh_renderer.draw(viewer, self.instance, self.instance.color, 0)
        if show_skeleton:
            self.skeleton_renderer.draw(viewer, self.instance, 0)
        else:
            self.skeleton_renderer.clear(viewer)
        if show_skeleton and show_axes:
    ...
    def clear(self, viewer) -> None:
        self.mesh_renderer.clear(viewer)
        self.skeleton_renderer.clear(viewer)
        self.coordinate_renderer.clear(viewer)
...
def play_motion(
    ...
    while viewer.is_running():
        ...
        overlay.draw(
            viewer,
            motion.bvh_local_transforms[frame_idx],
...
def main() -> None:
    ...
    parser.add_argument(
        "--show-skeleton", action="store_true", help="Draw human skeleton lines."
    )
    parser.add_argument(
